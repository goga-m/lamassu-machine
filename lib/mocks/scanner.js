...
function scanMainQR (cryptoCode, cryptoNetwork, callback) {
  _cancelCb = callback
  handle = setTimeout(function () {
    var resultStr = data.qrData[`${cryptoCode}:${cryptoNetwork}`] || data.qrData[`${cryptoCode}`]
    console.log('DEBUG405: %j', [cryptoCode, data.qrData])
    return callback(null, coinUtils.parseUrl(cryptoCode, cryptoNetwork, resultStr))
  }, 2000)
}
